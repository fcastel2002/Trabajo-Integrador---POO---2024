 ...
 * @param pantalla Referencia a una interfaz de pantalla para la interacci�n con el usuario.
 */
 ...
 * @return true si el comando se env�a correctamente, false en caso contrario.
 */
...
void Cliente::interpretarRespuesta(XmlRpcValue& respuesta) {
	...
	if (respuesta.getType() != XmlRpcValue::TypeArray) {
		errorHandler.logError("Respuesta no v�lida recibida del servidor.", ErrorLevel::WARNING);
 ...
 * @return Una cadena de texto con el contenido extra�do.
 */
std::string Cliente::extraerContenido(XmlRpcValue& contenido) {
	...
	// Elimina el �ltimo salto de l�nea si existe
	if (!resultado.empty()) {
		resultado.pop_back();
	}
...
std::vector<std::string> Cliente::pedirComandos(Orden& my_order) {
	...
	try {
		...
		if (result.getType() == XmlRpcValue::TypeArray) {
			...
			errorHandler.displayError("El servidor devolvi� un formato no esperado.", ErrorLevel::WARNING);
		}
...
/**
 * Realiza el proceso de inicio de sesi�n del usuario.
 */
void Cliente::login() {
